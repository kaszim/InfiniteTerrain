...
namespace InfiniteTerrain
{
    ...
    class PerlinNoise
    {
        ...
        private static readonly int repeat = 1;

        static PerlinNoise()
        {
        }

        public static float Get(Vector3 vec)
        {
            if (repeat > 0)
            ...
            var xi = (int)vec.X & 255;                              // Calculate the "unit cube" that the point asked will be located in
            var yi = (int)vec.Y & 255;                              // The left bound is ( |_x_|,|_y_|,|_z_| ) and the right bound is that
            var zi = (int)vec.Z & 255;                              // plus 1.  Next we calculate the location (from 0.0 to 1.0) in that cube.
            var xf = vec.X - (int)vec.X;
            var yf = vec.Y - (int)vec.Y;
            var zf = vec.Z - (int)vec.Z;
            ...
            return (MathHelper.Lerp(y1, y2, w) + 1) / 2;                      // For convenience we bind the result to 0 - 1 (theoretical min/max before is [-1, 1])
        }
